...
public class ActorResource {
	...
	@Transactional
	public List<ElementoShort<Integer, String>> getPeliculas(@PathVariable int id) throws NotFoundException {
		var item = srv.getOne(id);
		if(item.isEmpty())
			throw new NotFoundException();
		return item.get().getFilmActors().stream()
				.map(p -> new ElementoShort<Integer, String>(p.getFilm().getFilmId(), p.getFilm().getTitle()))
				.toList();
	}
